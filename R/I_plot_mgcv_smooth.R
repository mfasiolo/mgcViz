...
.plot.mgcv.smooth <- function(x, P, data=NULL, label="", se1.mult=1, se2.mult=2,
  ...
  if(x$dim == 1){
    out <- .plot.mgcv.smooth.1D(x=x, P=P, partial.resids=partial.resids, rug=rug, se=se, scale=scale, n=n,
                                jit=jit, shade=shade||(scheme==1), shade.col=shade.col, ylim = ylim,
                                shift=shift, trans=trans, by.resids=by.resids, inter=inter, ...)
  } else {
    if(x$dim == 2){
      out <- .plot.mgcv.smooth.2D(x=x, P=P, data=data, label=label, se.mult=se1.mult,
