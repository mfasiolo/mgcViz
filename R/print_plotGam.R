...
#' 
#' @description XXX
#' @param o an object of class \code{plotGam}.
...
#' @export 
#' 
print.plotGam <- function(o, ask = TRUE, pages = NULL, addLay = TRUE, ...){
  
  # If plots have no layers, we add some default ones
  if( addLay && o$empty ){
    
    o <- o + l_fitLine() + l_fitRaster() + l_fitContour() + l_ciLine() + l_rug()
  ...
  for(ii in 1:la$npag){
    ...
    tmp <- lapply(indx, function(.kk) o$plots[[.kk]]$ggObj)
