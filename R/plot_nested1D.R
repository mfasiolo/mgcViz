...
.plot.outer.nested.1D <- function(x, P, trans, maxpo) {
  ...
  if (!is.null(P$raw)) {
    ...
    .dat$res <- res[res$x >= P$xlim[1] &
                      res$x <= P$xlim[2], , drop = FALSE]
...
.plot.inner.nested.smooth.1D <- function(P, trans, maxpo, ci) {
  ...
  if (P$type == "mgks") {
    ...
    P$raw <- data.frame(z = P$p.resid,
                        x = P$X0[, 1],
                        y = P$X0[, 2])
    if (!is.null(P$raw)) {
      .dat$res <- P$raw[P$raw$x >= P$xlim[1] & P$raw$x <= P$xlim[2] &
                          P$raw$y >= P$ylim[1] &
                          P$raw$y <= P$ylim[2] , , drop = FALSE]
