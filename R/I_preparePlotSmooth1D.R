...
.preparePlotSmooth1D <- function(x, data, label="", se.mult=1, n=100, xlim = NULL, xlab=NULL, ylab=NULL, main=NULL, ...)
{
  out <- NULL
  if ( x$plot.me ){
    raw <- data[x$term][[1]]
    if (is.null(xlim)){ # Generate x sequence for prediction
      xx <- seq(min(raw), max(raw), length=n) } else {
    ...
    out <- list(X=X, x=xx, scale=TRUE, se=TRUE, raw=raw, xlab=xlabel, 
                ylab=ylabel, main=main, se.mult=se.mult, xlim=xlim)
  }
