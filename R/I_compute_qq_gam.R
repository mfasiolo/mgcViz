...
.compute.qq.gam.simul <- function(o, type, method, CI, level, rep, sortFun)
{
  ...
  return( list("D"=D, "Dq"=Dq, "dm"=dm, "conf"=conf, 
  ...
  if( !is.null(fam$cdf) ){
    ...
    if( CI != "none" ){
      ...
      if( method=="tnormal" ){ # Normal CI (from "Worm plot: a simple diagnostic device
        ...
        tmp <- 1:n
        conf <- rbind(qbeta(alpha, tmp, n - tmp + 1), qbeta(1-alpha, tmp, n - tmp + 1))
      }
    }
  }
  
  ylab <- ifelse(method=="tnormal", "Obs y transf to normal", "Obs y transf to unif")
  
  return( list("D"=D, "Dq"=Dq, "dm"=dm, "conf"=conf, "method"=method, "ylab"=ylab) )
}
