...
.spContour <- function(x, y, z, zse, xlab = "", ylab = "", zlab = "", titleOnly = FALSE,
  ...
  if (cs > 1) {
    cs <- 1 # text scaling based on height
  }
  tl <- strwidth(zlab)
  if (tl * cs > 3 * xr / 10) {
    cs <- (3 * xr / 10) / tl
  ...
  if (titleOnly) {
    ...
    text(xpos + xr / 10, ypos, zlab, xpd = TRUE,
         pos = 4, cex = cs * cm, off = 0.5 * cs * cm)
  }
  ...
  if (!titleOnly) {
    ...
    text(xpos + xr / 10, ypos, paste("-", round(se.mult), "se", sep = ""),
         xpd = TRUE, pos = 4, cex = cs * cm, off = 0.5 * cs * cm)
  }
  if (!"lty" %in% n.args) {
    args$lty <- 3
  }
